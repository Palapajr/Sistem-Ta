...
to_sql = pd.DataFrame(
    data_test.loc[:,['peserta','Nama_Pengurus','Anak SD','Anak SMP','Anak SMA','Ibu Hamil','Balita','Tahun']].values, 
    columns=['id_pkh','nama','sd','smp','sma','bumil','balita','tahun']
)
to_sql['kelas'] = hasil

# Insert data ke tabel testing di database
db_testing = db.Table('spk', meta, autoload=True, autoload_with=engine)
#to_sql.to_sql("spk", connection, if_exists='append', index=False, chunksize=50)
to_sql.to_csv("hasil.csv", index=False)