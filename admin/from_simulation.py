...
simulation = db.Table('simulation', meta, autoload=True, autoload_with=engine)

data_train = pd.read_sql(db.select([training]), connection)
data_test = pd.read_sql(db.select([simulation]), connection)
...
query = db.update(simulation).where(simulation.columns.id == 1).values(
    komp_prediksi=str(hasil[0]), 
    skor_Pendidikan=str(hasil_prob[0][1]),
    skor_Kesehatan=str(hasil_prob[0][0])
)
